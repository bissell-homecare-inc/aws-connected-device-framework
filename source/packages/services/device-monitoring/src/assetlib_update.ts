/*********************************************************************************************************************
 *  Copyright Amazon.com Inc. or its affiliates. All Rights Reserved.                                           *
 *                                                                                                                    *
 *  Licensed under the Apache License, Version 2.0 (the "License"). You may not use this file except in compliance    *
 *  with the License. A copy of the License is located at                                                             *
 *                                                                                                                    *
 *      http://www.apache.org/licenses/LICENSE-2.0                                                                    *
 *                                                                                                                    *
 *  or in the 'license' file accompanying this file. This file is distributed on an 'AS IS' BASIS, WITHOUT WARRANTIES *
 *  OR CONDITIONS OF ANY KIND, express or implied. See the License for the specific language governing permissions    *
 *  and limitations under the License.                                                                                *
 *********************************************************************************************************************/
import 'reflect-metadata';
import { inject, injectable } from 'inversify';
import {
  DevicesService,
  Device10Resource,
  ASSETLIBRARY_CLIENT_TYPES,
} from '@cdf/assetlibrary-client';
import { logger } from './utils/logger';

@injectable()
export class AssetLibUpdate {
  constructor(
    @inject(ASSETLIBRARY_CLIENT_TYPES.DevicesService) private readonly devices: DevicesService
  ) {}

  public async getDeviceConnected(deviceId: string): Promise<boolean> {
    logger.debug(`assetlib_update: getDeviceConnected: in: deviceId:${deviceId}`);

    try {
      // Get the device from asset library
      const device = await this.devices.getDeviceByID(deviceId);
      return device.connected ?? false;
    } catch (err) {
      logger.error(err);
    }

    logger.debug('assetlib_update: updatedevice: exit:');
    return undefined;
  }

  public async updateDeviceConnected(deviceId: string, connected: boolean): Promise<void> {
    logger.debug(
      `assetlib_update: updatedevice: in: deviceId:${deviceId}, connected:${connected}`
    );

    const updateRequest = {
      deviceId,
      connected,
    } as Device10Resource;

    try {
      // Get the device from asset library
      const device = await this.devices.getDeviceByID(deviceId);
      // Update only if necessary
      if (device.connected !== connected) {
        logger.debug(`Set device connected status to ${connected}`);
        await this.devices.updateDevice(deviceId, updateRequest);
      }
    } catch (err) {
      logger.error(err);
    }

    logger.debug('assetlib_update: updatedevice: exit:');
  }
}
